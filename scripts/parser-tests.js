...
const gameCases = [
  ...
  },
  {
    ...
    drive(game) {
      ...
      game.print(`Horrible goblin after escape: ${game.characters.horrible_goblin?.position || "none"}`);
      const lightStart = outputLines.length;
      game.flags.lanternon = false;
      game.flags.lanternturns = 0;
      game.execute("light lantern");
      const afterLight = outputLines.slice(lightStart);
      game.print(`Escape light fatal: ${game.endgame ? "yes" : "no"}`);
      game.print(`Escape light fight: ${afterLight.some((line) => /horrible goblin hurls|fight is done/i.test(line)) ? "yes" : "no"}`);
      const secondStart = outputLines.length;
    ...
    expectedIncluded: [
      ...
      "Horrible goblin after escape: big_cavern",
      "Escape light fatal: no",
      "Escape light fight: no",
  ...
  {
    name: "goblin hold answers attack on the greater goblin",
    setup(game) {
      movePlayerTo(game, "goblins_dungeon");
      placeCharacterWithPlayer(game, "thorin");
      placeCharacterWithPlayer(game, "gandalf");
    ...
    drive(game) {
      const start = outputLines.length;
      game.execute("kill goblin");
      const lines = outputLines.slice(start);
      game.print(`Hold attack answered: ${lines.some((line) => /bound with the rest|prison-rope first/i.test(line)) ? "yes" : "no"}`);
      game.print(`Hold attack absent: ${lines.some((line) => /no one named goblin/i.test(line)) ? "yes" : "no"}`);
      game.print(`Hold attack pressure: ${Number(game.flags.goblin_hold_pressure || 0)}`);
      const examStart = outputLines.length;
      game.execute("examine greater goblin");
      const exam = outputLines.slice(examStart);
      game.print(`Hold examine goblin: ${exam.some((line) => /greater goblin watches/i.test(line)) ? "yes" : "no"}`);
      // Sword in inventory used to make "cut rope" hit a legacy special action instead of the hold escape.
      giveItemToCharacter(game, "majestic_sword", game.player.id);
      game.flags.goblin_hold_kneeling = true;
      game.flags.goblin_hold_climbed = true;
      game.flags.goblin_hold_helper = "thorin";
      const cutStart = outputLines.length;
      game.execute("cut rope");
      const cutLines = outputLines.slice(cutStart);
      game.print(`Hold cut-with-sword room: ${game.currentRoom}`);
      game.print(`Hold cut-with-sword text: ${cutLines.some((line) => /Gandalf's voice cracks/i.test(line)) ? "yes" : "no"}`);
    },
    expectedIncluded: [
      "Hold attack answered: yes",
      "Hold attack absent: no",
      "Hold attack pressure: 2",
      "Hold examine goblin: yes",
      "Hold cut-with-sword room: dark_winding_passage",
      "Hold cut-with-sword text: yes",
    ],
  },
  {
